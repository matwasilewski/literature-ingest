...
@pytest.mark.parametrize(
    ...
    [
        [
            "oa_noncomm_xml.PMC000xxxxxx.baseline.invalid-date.filelist.csv"
        ]  # invalid date format
    ],
)
def test_get_baseline_date_failure(file_list):
    ...
    # Act/Assert
    with pytest.raises(
        ValueError, match="Found a file with `baseline` string but no date"
    ):
...
def test_parse_doc_basic_fields(pmc_doc):
    ...
    assert (
        doc.synthetic_id
        == "type=doi;id=10.1016/j.ijcchd.2022.100354&type=pmc;id=PMC10335194&type=pmid;id=37435574&type=pii;id=S2666-6685(22)00037-4"
    )
...
def test_parse_doc_authors(pmc_doc):
    ...
    assert (
        ...
        in first_author.affiliations[0]
    )
...
def test_parse_doc_content(pmc_doc):
    ...
    # Test abstract
    assert (
        "Pulmonary hypertension (PH) due to left heart disease" in doc.sections[1].text
    )
...
def test_document_to_raw_text(pmc_doc):
    ...
    assert (
        "Introduction\nPulmonary hypertension (PH) is defined as" in raw_text
    )  # From Introduction
    assert (
        "Methods\nWe performed a single center retrospective cohort study" in raw_text
    )  # From Methods
    assert (
        "Discussion\nIn this large cohort of pregnant women with PH" in raw_text
    )  # From Discussion
...
def test_doc_2(pmc_doc_2):
    ...
    assert doc.sections[1].text.startswith(
        "We analysed the genomic organisation of miR-153"
    )
    assert doc.keywords == [
        "Diabetes",
        "Glucose stimulation",
    ...
    assert doc.sections[2].text.startswith(
...
def test_document_load_from_json():
    ...
    # Verify dates
    assert (
        loaded_doc.publication_dates.received_date
        == original_doc.publication_dates.received_date
    )
    assert (
        loaded_doc.publication_dates.accepted_date
        == original_doc.publication_dates.accepted_date
    )
    assert (
        loaded_doc.publication_dates.epub_date
        == original_doc.publication_dates.epub_date
    )
...
def test_parse_doc_with_error(request, pmc_doc_fixture):
    ...
    doc = parser.parse_doc(
        request.getfixturevalue(pmc_doc_fixture), Path(f"{pmc_doc_fixture}.xml")
    )
